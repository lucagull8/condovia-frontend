import{useState,useEffect}from'react';import{Link}from'react-router-dom';import{TriangleAlert,ArrowRight,ChevronRight,Wallet,Building2,RefreshCw}from'lucide-react';import{Header,Footer,Badge}from'../components/Shared';import{ServiceIcon,findServiceCatalog}from'../components/ServiceIcon';import{useAuth}from'../context/AuthContext';import{getServizi,getCondomini}from'../api';
const fmt=n=>Number(n).toLocaleString('it-IT',{minimumFractionDigits:2});
export default function Home(){const{utente}=useAuth();const[servizi,setServizi]=useState([]);const[condomini,setCondomini]=useState([]);const[loadS,setLS]=useState(true);const[loadC,setLC]=useState(true);const[filtro,setFiltro]=useState(null);const[refreshing,setRefreshing]=useState(false);
const loadData=()=>{setLS(true);setLC(true);Promise.all([getServizi(),getCondomini()]).then(([s,c])=>{setServizi(s);setCondomini(c);}).finally(()=>{setLS(false);setLC(false);setRefreshing(false);});};
useEffect(()=>{loadData();},[]);
...
<div style={{display:'flex',alignItems:'center',gap:10,marginTop:2}}><p style={{margin:0,fontSize:14,color:'var(--ink-soft)'}}>{studio} · {condomini.length} condomini gestiti</p><button onClick={()=>{setRefreshing(true);loadData();}} disabled={refreshing} title="Aggiorna" style={{display:'flex',alignItems:'center',gap:4,height:26,padding:'0 8px',borderRadius:6,border:'1px solid var(--border)',background:'transparent',fontSize:11.5,cursor:'pointer',color:'var(--ink-soft)'}}><RefreshCw size={11} style={{animation:refreshing?'spin 1s linear infinite':'none'}}/>{refreshing?'…':'Aggiorna'}</button><style>{`@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}`}</style></div></div>
